...
if __name__ == '__main__':
    ...
    train_data = torch.from_numpy(train_data[:, ::10, :]).float()
    test1_data = torch.from_numpy(test1_data[:, ::10, :]).float()
    test2_data = torch.from_numpy(test2_data[:, ::10, :]).float()

    # check for outliers and set values equal to zero
    train_data[np.where(train_data > 2)] = 0
    test1_data[np.where(test1_data > 2)] = 0
    test2_data[np.where(test2_data > 2)] = 0
    ...
    filename = 'CNN_nosubtract_n' + str(n_iter) + '_rw' + str(rw) + '_lr' + str(lr) + '_do' + str(drop_rate) + '.pt'
